...
int main(int argc, char** argv) {
	...
	if(nh_agent1.getParam("flag_use_gazebo", use_gazebo)) {}

	//============== FOR SINE FOLLOWER =================
	path = PP.sine_follower(1,5);
	...
	// Retrieve the states of the agent either from GAZEBO or OPTITRACK
	if(use_gazebo) {
		if(nh_agent1.getParam("/GazeboInfo/Agent_1_Yaw", yaw)) {}
		if(nh_agent1.getParam("/GazeboInfo/Agent_1_X_Coordinate", x_cor)) {}
		if(nh_agent1.getParam("/GazeboInfo/Agent_1_Y_Coordinate", y_cor)) {}
  ...
  time_start = ros::Time::now().toSec();
	...
	while(distanceToGoal > goalRadius) {
		...
		// Retrieve the states of the agent either from GAZEBO or OPTITRACK
		if(use_gazebo) {
			if(nh_agent1.getParam("/GazeboInfo/Agent_1_Yaw", yaw_new)) {}
			if(nh_agent1.getParam("/GazeboInfo/Agent_1_X_Coordinate", x_cor_new)) {}
			if(nh_agent1.getParam("/GazeboInfo/Agent_1_Y_Coordinate", y_cor_new)) {}
