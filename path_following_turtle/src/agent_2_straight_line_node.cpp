...
int main(int argc, char** argv) {
	...
	if(nh.getParam("GazeboInfo/Agent_2_Yaw", yaw)) {}
	if(nh.getParam("GazeboInfo/Agent_2_X_Coordinate", x_cor)) {}
	if(nh.getParam("GazeboInfo/Agent_2_Y_Coordinate", y_cor)) {}

	curPose << x_cor << y_cor << yaw << endr;

	//============== FOR LINE FOLLOWER =================
	path = PP.line_follower(0,-5,6.2657,-5);
	delta = 0.01;
	path2follow = path;
	path2follow.save("VAR_Agent_2_Path-2-Follow.csv", csv_ascii);
	robotGoal = path2follow.row(1); // For Line following
	//============== =============== =================
	...
	while(distanceToGoal > goalRadius) {
		...
		// Retrieve the states of the agent from OPTITRACK
		if(nh.getParam("GazeboInfo/Agent_2_Yaw", yaw_new)) {}
		if(nh.getParam("GazeboInfo/Agent_2_X_Coordinate", x_cor_new)) {}
		if(nh.getParam("GazeboInfo/Agent_2_Y_Coordinate", y_cor_new)) {}
